 ...
 * @param {string} trackId spotify access token
 * @return {object} quantifier for song sentiment from 0 to 1
 */
export const songSentiment = async (artist, song, duration, trackId, token) => {
  const text = await lyrics(artist, song)
  const totalSentiment = sentiment(text)

  const totalValence = await trackFeatures(trackId, token)
  const totalNegativity = totalSentiment.negative.length / totalSentiment.words.length
 ...
 */
export const trackFeatures = (id, token) => SpotifyAPI.audioFeatures(id, token).then(res => res.valence)
