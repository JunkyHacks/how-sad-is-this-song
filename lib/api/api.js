...
const retrieveIdsFromTrackList = tracks => tracks.map(track => track.id)
 ...
 */
export const search = async (text, type) => await SpotifyAPI.search({ q: text, type: type }, limit)
      ...
      albums.items.map(async (album, indexA) => {
        ...
        await Promise.all(
          albums.items[indexA].tracks.map(async (track, indexT) => {
            const features = await SpotifyAPI.audioFeatures(retrieveIdsFromTrackList(tracks.items)).then(
              features => features.audio_features,
            )
            album.items[indexA].tracks[indexT].features = features[indexT]
          }),
        )
      }),
    )
